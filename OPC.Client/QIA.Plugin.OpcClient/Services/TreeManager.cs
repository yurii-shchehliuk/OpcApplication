...
namespace QIA.Plugin.OpcClient.Services
{
    public class TreeManager : ITreeManager
    {
        ...
        /// </summary>
        public TreeNode<NodeData> BrowseTree()
        {
            ...
            var treeNode = new TreeNode<NodeData>(new()
            {
                Name = parentName,
            ...
            void BrowseFoldersFunc(ReferenceDescription rd, HashSet<TreeNode<NodeData>> treeNodeList)
            {
                deepLvl++;
                ...
                treeNodeList.Add(new TreeNode<NodeData>(new()
                {
        ...
        /// <returns></returns>
        public TreeNode<NodeData> FindNodesRecursively(IAppSettings newAppSettings = null)
        {
            int deepLvl = -1;
            string parentName = "ROOT";
            itemsToFind = newAppSettings.NodeManager;

            if (itemsToFind is null)
                return null;

            TreeNode<NodeData> treeNode = new(new()
            {
            ...
            void FindNodesRecursivelyFunc(ReferenceDescription rd, HashSet<TreeNode<NodeData>> treeNodeList)
            {
                deepLvl++;
                ...
                /// add same row children
                var currentNode = new TreeNode<NodeData>(new NodeData
                {
                    NodeId = currentNodeId,
                ...
                if (foundItem is not null)
                {
                    ...
                    _azureMS.SendMessageAsync(node.Data).Wait();
                }
            ...
            bool HasNeededChild(HashSet<TreeNode<NodeData>> treeNode, HashSet<NodeConfig> itemsToFind = null)
            {
        ...
        public async Task WriteTree(string graphName, TreeNode<NodeData> treeData)
        {
            await File.WriteAllTextAsync(graphName, JsonSerializer.Serialize(treeData));
