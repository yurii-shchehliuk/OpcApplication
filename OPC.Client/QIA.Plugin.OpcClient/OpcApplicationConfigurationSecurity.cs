...
namespace QIA.Plugin.OpcClient
{
    ...
    public partial class OpcApplicationConfiguration
    {
        ...
        public static string GetApplicationUriFromCertificate(X509Certificate2 certificate)
        {
            ...
            //var sanExtension = certificate.Uris["2.5.29.17"]; // OID for Subject Alternative Name
            //if (sanExtension != null)
            //{
            //	var asnData = new Asn1Reader(sanExtension.RawData);
            //	if (asnData.MoveNext() && asnData.Tag == (int)Asn1Type.SEQUENCE)
            //	{
            //		while (asnData.MoveNext())
            //		{
            //			// We're looking for a URI
            //			if (asnData.Tag == (int)Asn1Type.IA5String)
            //			{
            //				applicationUri = asnData.GetPayload().ToArray().Select(b => (char)b).ToArray();
            //				break;
            //			}
            //		}
            //	}
