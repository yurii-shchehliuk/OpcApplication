...
public class AutoMapperProfile : Profile
{
    ...
    private void MapSessions()
    {
        ...
        CreateMap<SessionConfig, SessionResponse>()
          .ForMember(dest => dest.Guid, opt => opt.MapFrom(src => src.Guid));
    }
    ...
    private static string ValueMapExtension(MonitoredItem item)
    {
        try
        {
            IList<DataValue> result = item.DequeueValues();
            if (result.Count == 0)
            {
                ...
                dynamic data = (item.LastValue as dynamic).Value.Value.ToString();
                return data ?? "0";
            }
    ...
    private static int OpcIdMapExtension(MonitoredItem item)
    {
        try
        {
            var id = int.Parse(item.ServerId.ToString());
            return id;
        }
        catch (Exception ex)
        {
