...
public partial class ApplicationConfigBuilder
{
    ...
    /// </summary>
    private static readonly List<string> IssuerCertificateBase64Strings;
    private static readonly List<string> IssuerCertificateFileNames;

    /// <summary>
    /// Trusted certificates to add.
    /// </summary>
    private static readonly List<string> TrustedCertificateBase64Strings;
    private static readonly List<string> TrustedCertificateFileNames;
    ...
    /// </summary>
    private static readonly List<string> ThumbprintsToRemove;
    #endregion
    ...
    private async Task LoadAppArguments()
    {
    ...
    /// </summary>
    public async Task InitCertificateAsync()
    {
        ...
        ApplicationConfiguration.CertificateValidator.CertificateValidation += new CertificateValidationEventHandler(CertificateValidatorCertificateValidation);
        ...
        if (certificate == null)
        {
            ...
            LoggerManager.Logger.Information($"Valid From: {certificate.NotBefore}");
            LoggerManager.Logger.Information($"Valid Until: {certificate.NotAfter}");
        }
        else
        {
    ...
    public X509Certificate2 CreateSelfSignedCertificate()
    {
        ...
        CertificateRequest request = new("cn=" + ApplicationConfiguration.ApplicationName, rsa, HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1);

        // Adding key usage extensions
        request.CertificateExtensions.Add(
            new X509KeyUsageExtension(X509KeyUsageFlags.DataEncipherment | X509KeyUsageFlags.KeyEncipherment | X509KeyUsageFlags.DigitalSignature, false));

        // Create the certificate valid for 1 year
        X509Certificate2 certificate = request.CreateSelfSigned(new DateTimeOffset(DateTime.UtcNow.AddDays(-31)), new DateTimeOffset(DateTime.UtcNow.AddYears(1)));

        // Export the certificate with the private key (you might need to secure the output)
        return certificate;
    }
    ...
    public async Task ShowCreateSigningRequestInformationAsync(X509Certificate2 certificate)
    {
        try
        {
            ...
            if (ApplicationConfiguration.ApplicationType != ApplicationType.Client)
            {
                ...
                var serverCapabilities = ApplicationConfiguration.ServerConfiguration.ServerCapabilities.ToArray();
    ...
    public async Task ShowCertificateStoreInformationAsync()
    {
        ...
        try
        {
            ...
            X509Certificate2Collection certs = await certStore.Enumerate();
            var certNum = 1;
            ...
            {
                X509CRLCollection crls = await certStore.EnumerateCRLs();
                var crlNum = 1;
                LoggerManager.Logger.Information($"Trusted issuer store has {crls.Count} CRLs.");
                foreach (X509CRL crl in await certStore.EnumerateCRLs())
                {
    ...
    /// </summary>
    public void CertificateValidatorCertificateValidation(CertificateValidator validator, CertificateValidationEventArgs e)
    {
    ...
    private async Task<bool> RemoveCertificatesAsync(List<string> thumbprintsToRemove)
    {
        ...
        try
        {
            ...
            {
                X509Certificate2Collection certToRemove = await trustedStore.FindByThumbprint(thumbprint);
        ...
        try
        {
            ...
            foreach (var thumbprint in thumbprintsToRemove)
            {
                X509Certificate2Collection certToRemove = await issuerStore.FindByThumbprint(thumbprint);
                if (certToRemove != null && certToRemove.Count > 0)
                {
                    ...
                    bool issuerCertificate = true)
    {
        ...
        try
        {
            ...
            if (certificateFileNames?.Count > 0)
            {
                ...
                {
                    X509Certificate2 certificate = new(certificateFileName);
            ...
            if (certificateBase64Strings?.Count > 0)
            {
                ...
                {
                    var buffer = new byte[certificateBase64String.Length * 3 / 4];
                    if (Convert.TryFromBase64String(certificateBase64String, buffer, out var written))
                    {
                        X509Certificate2 certificate = new(buffer);
                        certificatesToAdd.Add(certificate);
                    }
                    else
                    {
        ...
        if (issuerCertificate)
        {
            ...
            {
                using ICertificateStore issuerStore = CertificateStoreIdentifier.OpenStore(ApplicationConfiguration.SecurityConfiguration.TrustedIssuerCertificates.StorePath);
                foreach (X509Certificate2 certificateToAdd in certificatesToAdd)
                {
                    try
                    {
    ...
    private async Task<bool> UpdateCrlAsync(string newCrlBase64String, string newCrlFileName)
    {
        ...
        try
        {
            ...
            {
                var buffer = new byte[newCrlBase64String.Length * 3 / 4];
                if (Convert.TryFromBase64String(newCrlBase64String, buffer, out var written))
                {
                    newCrl = new X509CRL(buffer);
                }
                else
                {
                    LoggerManager.Logger.Error($"The provided string '{newCrlBase64String[..10]}...' is not a valid base64 string.");
                    return false;
                }
            }
            else
        ...
        using (ICertificateStore trustedStore = CertificateStoreIdentifier.OpenStore(ApplicationConfiguration.SecurityConfiguration.TrustedPeerCertificates.StorePath))
        {
            var trustedCrlIssuer = false;
            X509Certificate2Collection trustedCertificates = await trustedStore.Enumerate();
            foreach (X509Certificate2 trustedCertificate in trustedCertificates)
            {
                try
                {
                    if (CompareDistinguishedName(newCrl.Issuer, trustedCertificate.Subject) && newCrl.VerifySignature(trustedCertificate, false))
                    {
                        ...
                        X509CRLCollection crlsToRemove = await trustedStore.EnumerateCRLs(trustedCertificate);
                        foreach (X509CRL crlToRemove in crlsToRemove)
                        {
                            try
                            {
        ...
        {
            var trustedCrlIssuer = false;
            X509Certificate2Collection issuerCertificates = await issuerStore.Enumerate();
            foreach (X509Certificate2 issuerCertificate in issuerCertificates)
            {
                try
                {
                    if (CompareDistinguishedName(newCrl.Issuer, issuerCertificate.Subject) && newCrl.VerifySignature(issuerCertificate, false))
                    {
                        ...
                        trustedCrlIssuer = true;
                        X509CRLCollection crlsToRemove = await issuerStore.EnumerateCRLs(issuerCertificate);
                        foreach (X509CRL crlToRemove in crlsToRemove)
                        {
                            try
                            {
                    ...
                    string privateKeyFileName)
    {
        ...
        try
        {
            ...
            {
                var buffer = new byte[newCertificateBase64String.Length * 3 / 4];
                if (Convert.TryFromBase64String(newCertificateBase64String, buffer, out var written))
                {
                    newCertificate = new X509Certificate2(buffer);
                }
                else
                {
                    LoggerManager.Logger.Error($"The provided string '{newCertificateBase64String[..10]}...' is not a valid base64 string.");
                    return false;
                }
            }
            else
        ...
        try
        {
            if (!string.IsNullOrEmpty(privateKeyBase64String))
            {
                ...
                if (!Convert.TryFromBase64String(privateKeyBase64String, privateKey, out var written))
                {
                    LoggerManager.Logger.Error($"The provided string '{privateKeyBase64String[..10]}...' is not a valid base64 string.");
                    return false;
                }
            }
        ...
        var hasApplicationCertificate = false;
        ...
        try
        {
            if (!CompareDistinguishedName(newCertificate.Subject, newCertificate.Issuer))
            {
                ...
                CertificateValidator certValidator = new();
                CertificateTrustList verificationTrustList = new();
                CertificateIdentifierCollection verificationCollection = new();
                using (ICertificateStore issuerStore = CertificateStoreIdentifier.OpenStore(ApplicationConfiguration.SecurityConfiguration.TrustedIssuerCertificates.StorePath))
                {
                    X509Certificate2Collection certs = await issuerStore.Enumerate();
                    foreach (X509Certificate2 cert in certs)
                    {
                ...
                using (ICertificateStore trustedStore = CertificateStoreIdentifier.OpenStore(ApplicationConfiguration.SecurityConfiguration.TrustedPeerCertificates.StorePath))
                {
                    X509Certificate2Collection certs = await trustedStore.Enumerate();
                    foreach (X509Certificate2 cert in certs)
                    {
                        verificationCollection.Add(new CertificateIdentifier(cert));
                    }
        ...
        if (string.IsNullOrEmpty(newCertFormat))
        {
            try
            {
                X509Certificate2 certWithPrivateKey = new(privateKey, certificatePassword);
    ...
    public static bool CompareDistinguishedName(string dn1, string dn2)
    {
