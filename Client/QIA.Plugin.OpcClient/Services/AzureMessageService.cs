...
namespace QIA.Plugin.OpcClient.Services
{
    public class AzureMessageService : IAzureMessageService
    {
        ...
        public async Task SendMessageAsync(SampleNode message)
        {
            try
            {
                //TODO: split with services and repos interfaces
                if (!Extensions.ReadSettings<AppSettings>().SaveToAzure)
                {
                    return;
                }

                LoggerManager.Logger.Information(string.Format(" > Sending node: {0} {1}", message.Name, message.NodeId));
                ...
                LoggerManager.Logger.Information(string.Format(" < Message send"));
            }
            catch (Exception ex)
            {
