...
namespace OpcPlc
{
    ...
    public partial class OpcApplicationConfiguration
    {
        ...
        /// </summary>
        public static List<string> DnsNames = new List<string>();

        /// <summary>
        /// Configures OPC stack security.
        /// </summary>
        public ApplicationConfiguration InitApplicationSecurityAsync(IApplicationConfigurationBuilderServerOptions securityBuilder)
        {
            ...
            ApplicationConfiguration = options.Create().Result;

            Console.WriteLine($"Application Certificate store type is: {id.StoreType}");
            Console.WriteLine($"Application Certificate store path is: {id.StorePath}");

            Console.WriteLine($"Rejection of SHA1 signed certificates is {securityConfiguration.RejectSHA1SignedCertificates}");

            Console.WriteLine($"Minimum certificate key size set to {securityConfiguration.MinimumCertificateKeySize}");

            //Console.WriteLine("Trusted Issuer store type is: {storeType}" + issuerList.StoreType);
            //Console.WriteLine("Trusted Issuer Certificate store path is: {storePath}" + issuerList.StorePath);

            //Console.WriteLine("Trusted Peer Certificate store type is: {storeType}" + trustList.StoreType);
            //Console.WriteLine("Trusted Peer Certificate store path is: {storePath}" + trustList.StorePath);

            //Console.WriteLine("Rejected certificate store type is: {storeType}" + rejectedStore.StoreType);
            //Console.WriteLine("Rejected Certificate store path is: {storePath}" + rejectedStore.StorePath);

            // handle cert validation
            if (AutoAcceptCerts)
            {
                Console.WriteLine("Automatically accepting all client certificates, this is a security risk!");
            }
        ...
        public static async Task ShowCreateSigningRequestInformationAsync(X509Certificate2 certificate)
        {
            try
            {
                ...
                if (!certificate.HasPrivateKey)
                {
                    ...
                    {
                        Console.WriteLine(e.Message + "Error while loading private key");
                        return;
                    }
                }
                ...
                {
                    Console.WriteLine(e.Message, "Error while creating signing request");
                    return;
                }
                Console.WriteLine("----------------------- CreateSigningRequest information ------------------");
                Console.WriteLine("ApplicationUri: {applicationUri}" + ApplicationConfiguration.ApplicationUri);
                Console.WriteLine("ApplicationName: {applicationName}" + ApplicationConfiguration.ApplicationName);
                Console.WriteLine("ApplicationType: {applicationType}" + ApplicationConfiguration.ApplicationType);
                Console.WriteLine("ProductUri: {productUri}" + ApplicationConfiguration.ProductUri);
                if (ApplicationConfiguration.ApplicationType != ApplicationType.Client)
                {
                    ...
                    {
                        Console.WriteLine($"DiscoveryUrl[{serverNum++}]: {endpoint}");
                    }
                    ...
                    Console.WriteLine($"ServerCapabilities: {string.Join(" + ", serverCapabilities)}");
                }
                Console.WriteLine("CSR (base64 encoded):");
                Console.WriteLine($"{Convert.ToBase64String(certificateSigningRequest)}");
                Console.WriteLine("---------------------------------------------------------------------------");
                try
                {
                    File.WriteAllBytes($"{ApplicationConfiguration.ApplicationName}.csr", certificateSigningRequest);
                    Console.WriteLine($"Binary CSR written to '{ApplicationConfiguration.ApplicationName}.csr'");
                }
                catch (Exception e)
                {
                    Console.WriteLine(e.Message + "Error while writing .csr file");
                }
            }
            catch (Exception e)
            {
                Console.WriteLine(e.Message + "Error in CSR creation");
            }
        }
        ...
        public static async Task ShowCertificateStoreInformationAsync()
        {
            ...
            try
            {
                ...
                int certNum = 1;
                Console.WriteLine("Application store contains {count} certs" + certs.Count);
                foreach (var cert in certs)
                {
                    Console.WriteLine("{index}: Subject {subject} (thumbprint: {thumbprint})" +
                        $"{certNum++:D2}" +
                        cert.Subject +
            ...
            {
                Console.WriteLine(e.Message + "Error while trying to read information from application store");
            }
            ...
            try
            {
                ...
                var certs = await certStore.Enumerate().ConfigureAwait(false);
                int certNum = 1;
                Console.WriteLine("Trusted issuer store contains {count} certs" + certs.Count);
                foreach (var cert in certs)
                {
                    Console.WriteLine("{index}: Subject {subject} (thumbprint: {thumbprint})" + $"{certNum++:D2}" + cert.Subject + cert.GetCertHashString());
                }
                ...
                if (certStore.SupportsCRLs)
                {
                    ...
                    int crlNum = 1;
                    Console.WriteLine("Trusted issuer store has {count} CRLs" + crls.Count);
                    foreach (var crl in crls)
                    {
                        Console.WriteLine("{index}: Issuer {issuer}, Next update time {nextUpdate}" +
                            $"{crlNum++:D2}" +
                            crl.Issuer +
                            crl.NextUpdate);
                    }
            ...
            {
                Console.WriteLine(e.Message + "Error while trying to read information from trusted issuer store");
            }
            ...
            try
            {
                ...
                var certs = await certStore.Enumerate().ConfigureAwait(false);
                int certNum = 1;
                Console.WriteLine("Trusted peer store contains {count} certs" + certs.Count);
                foreach (var cert in certs)
                {
                    Console.WriteLine("{index}: Subject {subject} (thumbprint: {thumbprint})" + $"{certNum++:D2}" + cert.Subject + cert.GetCertHashString());
                }
                ...
                if (certStore.SupportsCRLs)
                {
                    var crls = await certStore.EnumerateCRLs().ConfigureAwait(false);
                    int crlNum = 1;
                    Console.WriteLine("Trusted peer store has {count} CRLs" + crls.Count);
                    foreach (var crl in crls)
                    {
                        Console.WriteLine("{index}: Issuer {issuer}, Next update time {nextUpdate}" + $"{crlNum++:D2}" + crl.Issuer + crl.NextUpdate);
                    }
                }
            }
            catch (Exception e)
            {
                Console.WriteLine(e.Message, "Error while trying to read information from trusted peer store");
            }
            ...
            try
            {
                ...
                var certs = await certStore.Enumerate().ConfigureAwait(false);
                int certNum = 1;
                Console.WriteLine("Rejected certificate store contains {count} certs" + certs.Count);
                foreach (var cert in certs)
                {
                    Console.WriteLine("{index}: Subject {subject} (thumbprint: {thumbprint})" +
                        $"{certNum++:D2}" +
            ...
            {
                Console.WriteLine(e.Message, "Error while trying to read information from rejected certificate store");
            }
        }
        ...
        private static void CertificateValidator_CertificateValidation(CertificateValidator validator, CertificateValidationEventArgs e)
        {
            if (e.Error.StatusCode == StatusCodes.BadCertificateUntrusted)
            {
                ...
                if (AutoAcceptCerts)
                {
                    Console.WriteLine("Trusting certificate {certificateSubject} because of corresponding command line option" + e.Certificate.Subject);
                }
                else
                {
                    Console.WriteLine(
        ...
        private async Task<bool> RemoveCertificatesAsync(List<string> thumbprintsToRemove)
        {
            ...
            if (thumbprintsToRemove.Count == 0)
            {
                Console.WriteLine("There is no thumbprint specified for certificates to remove. Please check your command line options.");
                return false;
            }
            ...
            try
            {
                Console.WriteLine("Starting to remove certificate(s) from trusted peer and trusted issuer store");
                ...
                foreach (var thumbprint in thumbprintsToRemove)
                {
                    ...
                    if (certToRemove?.Count > 0)
                    {
                        ...
                        {
                            Console.WriteLine($"Failed to remove certificate with thumbprint '{thumbprint}' from the trusted peer store");
                        }
                        else
                        {
                            Console.WriteLine($"Removed certificate with thumbprint '{thumbprint}' from the trusted peer store");
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine(e.Message, "Error while trying to remove certificate(s) from the trusted peer store");
                result = false;
            }
            ...
            try
            {
                ...
                foreach (var thumbprint in thumbprintsToRemove)
                {
                    ...
                    if (certToRemove?.Count > 0)
                    {
                        ...
                        {
                            Console.WriteLine($"Failed to delete certificate with thumbprint '{thumbprint}' from the trusted issuer store");
                        }
                        else
                        {
                            Console.WriteLine($"Removed certificate with thumbprint '{thumbprint}' from the trusted issuer store");
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine(e.Message, "Error while trying to remove certificate(s) from the trusted issuer store");
                result = false;
            }
            return result;
        }
            ...
            bool issuerCertificate = true)
        {
            ...
            {
                Console.WriteLine("There is no certificate provided. Please check your command line options.");
                return false;
            }

            Console.WriteLine($"Starting to add certificate(s) to the {(issuerCertificate ? "trusted issuer" : "trusted peer")} store");
            ...
            try
            {
                ...
                if (certificateBase64Strings?.Count > 0)
                {
                    foreach (var certificateBase64String in certificateBase64Strings)
                    {
                        ...
                        {
                            Console.WriteLine($"The provided string '{certificateBase64String.Substring(0, 10)}...' is not a valid base64 string");
                            return false;
                        }
                    }
                }
            ...
            {
                Console.WriteLine(e.Message, "The issuer certificate data is invalid. Please check your command line options");
                return false;
            }
                ...
                try
                {
                    ...
                    foreach (var certificateToAdd in certificatesToAdd)
                    {
                        try
                        {
                            ...
                            Console.WriteLine($"Certificate '{certificateToAdd.SubjectName.Name}' and thumbprint '{certificateToAdd.Thumbprint}' was added to the trusted issuer store");
                        }
                        catch (ArgumentException)
                        {
                            // ignore error if cert already exists in store
                            Console.WriteLine($"Certificate '{certificateToAdd.SubjectName.Name}' already exists in trusted issuer store");
                        }
                    }
                }
                catch (Exception e)
                {
                    Console.WriteLine(e.Message, "Error while adding a certificate to the trusted issuer store");
                    result = false;
                }
            }
            else
                ...
                try
                {
                    ...
                    foreach (var certificateToAdd in certificatesToAdd)
                    {
                        try
                        {
                            ...
                            Console.WriteLine($"Certificate '{certificateToAdd.SubjectName.Name}' and thumbprint '{certificateToAdd.Thumbprint}' was added to the trusted peer store");
                        }
                        catch (ArgumentException)
                        {
                            // ignore error if cert already exists in store
                            Console.WriteLine($"Certificate '{certificateToAdd.SubjectName.Name}' already exists in trusted peer store");
                        }
                    }
                }
                catch (Exception e)
                {
                    Console.WriteLine(e.Message, "Error while adding a certificate to the trusted peer store");
                    result = false;
                }
            }
            return result;
        ...
        private async Task<bool> UpdateCrlAsync(string newCrlBase64String, string newCrlFileName)
        {
            ...
            {
                Console.WriteLine("There is no CRL specified. Please check your command line options");
                return false;
            }

            // validate input and create the new CRL
            Console.WriteLine("Starting to update the current CRL");
            X509CRL newCrl;
            try
            {
                if (string.IsNullOrEmpty(newCrlFileName))
                {
                    ...
                    {
                        Console.WriteLine($"The provided string '{newCrlBase64String.Substring(0, 10)}...' is not a valid base64 string");
                        return false;
                    }
                }
                else
            ...
            {
                Console.WriteLine(e.Message, "The new CRL data is invalid");
                return false;
            }
            ...
            using (ICertificateStore trustedStore = CertificateStoreIdentifier.OpenStore(ApplicationConfiguration.SecurityConfiguration.TrustedPeerCertificates.StorePath))
            {
                ...
                foreach (var trustedCertificate in trustedCertificates)
                {
                    try
                    {
                        if (X509Utils.CompareDistinguishedName(newCrl.Issuer, trustedCertificate.Subject) && newCrl.VerifySignature(trustedCertificate, false))
                        {
                            ...
                            Console.WriteLine("Remove the current CRL from the trusted peer store");
                            ...
                            foreach (var crlToRemove in crlsToRemove)
                            {
                                try
                                {
                                    ...
                                    {
                                        Console.WriteLine($"Failed to remove CRL issued by '{crlToRemove.Issuer}' from the trusted peer store");
                                    }
                                }
                                catch (Exception e)
                                {
                                    Console.WriteLine(e.Message, $"Error while removing the current CRL issued by '{crlToRemove.Issuer}' from the trusted peer store");
                                    result = false;
                                }
                            }
                        }
                    ...
                    {
                        Console.WriteLine(e.Message, "Error while removing the cureent CRL from the trusted peer store");
                        result = false;
                    }
                }
                ...
                if (trustedCrlIssuer)
                {
                    try
                    {
                        ...
                        Console.WriteLine($"The new CRL issued by '{newCrl.Issuer}' was added to the trusted peer store");
                    }
                    catch (Exception e)
                    {
                        Console.WriteLine(e.Message, "Error while adding the new CRL to the trusted peer store");
                        result = false;
                    }
                }
            }
            ...
            using (ICertificateStore issuerStore = CertificateStoreIdentifier.OpenStore(ApplicationConfiguration.SecurityConfiguration.TrustedIssuerCertificates.StorePath))
            {
                ...
                foreach (var issuerCertificate in issuerCertificates)
                {
                    try
                    {
                        ...
                        {
                            // the issuer of the new CRL is trusted. delete the crls of the issuer in the trusted store
                            Console.WriteLine("Remove the current CRL from the trusted issuer store");
                            trustedCrlIssuer = true;
                            ...
                            foreach (var crlToRemove in crlsToRemove)
                            {
                                try
                                {
                                    ...
                                    {
                                        Console.WriteLine($"Failed to remove the current CRL issued by '{crlToRemove.Issuer}' from the trusted issuer store");
                                    }
                                }
                                catch (Exception e)
                                {
                                    Console.WriteLine(e.Message, $"Error while removing the current CRL issued by '{crlToRemove.Issuer}' from the trusted issuer store");
                                    result = false;
                                }
                            }
                        }
                    ...
                    {
                        Console.WriteLine(e.Message, "Error while removing the current CRL from the trusted issuer store");
                        result = false;
                    }
                }
                ...
                if (trustedCrlIssuer)
                {
                    try
                    {
                        ...
                        Console.WriteLine($"The new CRL issued by '{newCrl.Issuer}' was added to the trusted issuer store");
                    }
                    catch (Exception e)
                    {
                        Console.WriteLine(e.Message, $"Error while adding the new CRL issued by '{newCrl.Issuer}' to the trusted issuer store");
                        result = false;
                    }
                }
            }
            ...
            string privateKeyFileName)
        {
            ...
            {
                Console.WriteLine("There is no new application certificate data provided. Please check your command line options.");
                return false;
            }
            ...
            try
            {
                if (string.IsNullOrEmpty(newCertificateFileName))
                {
                    ...
                    {
                        Console.WriteLine($"The provided string '{newCertificateBase64String.Substring(0, 10)}...' is not a valid base64 string");
                        return false;
                    }
                }
                else
            ...
            {
                Console.WriteLine(e.Message, "The new application certificate data is invalid");
                return false;
            }

            // validate input and create the private key
            Console.WriteLine("Start updating the current application certificate");
            ...
            try
            {
                if (!string.IsNullOrEmpty(privateKeyBase64String))
                {
                    ...
                    {
                        Console.WriteLine($"The provided string '{privateKeyBase64String.Substring(0, 10)}...' is not a valid base64 string");
                        return false;
                    }
                }
            ...
            {
                Console.WriteLine(e.Message, "The private key data is invalid");
                return false;
            }
            ...
            if (ApplicationConfiguration.SecurityConfiguration.ApplicationCertificate?.Certificate != null)
            {
                ...
                Console.WriteLine($"The current application certificate has SubjectName '{currentSubjectName}' and thumbprint '{currentApplicationCertificate.Thumbprint}'");
            }
            else
            {
                Console.WriteLine("There is no existing application certificate");
            }
            ...
            {
                Console.WriteLine($"The SubjectName '{newCertificate.SubjectName.Name}' of the new certificate doesn't match the current certificates SubjectName '{currentSubjectName}'");
                return false;
            }
            ...
            {
                Console.WriteLine(e.Message, "Failed to verify integrity of the new certificate and the trusted issuer list");
                return false;
            }
            ...
            if (string.IsNullOrEmpty(newCertFormat))
            {
                try
                {
                    ...
                    newCertFormat = "PFX";
                    Console.WriteLine("The private key for the new certificate was passed in using PFX format");
                }
                catch
                {
                    Console.WriteLine("Certificate file is not PFX");
                }
            }
            ...
            if (string.IsNullOrEmpty(newCertFormat))
            {
                try
                {
                    ...
                    newCertFormat = "PEM";
                    Console.WriteLine("The private key for the new certificate was passed in using PEM format");
                }
                catch
                {
                    Console.WriteLine("Certificate file is not PEM");
                }
            }
            if (string.IsNullOrEmpty(newCertFormat))
            {
                ...
                try
                {
                    ...
                    {
                        Console.WriteLine("There is no existing application certificate we can use to extract the private key. You need to pass in a private key using PFX or PEM format");
                    }
                }
                catch
                {
                    Console.WriteLine("Application certificate format is not DER");
                }
            }
            ...
            if (hasApplicationCertificate)
            {
                if (string.IsNullOrEmpty(newCertFormat))
                {
                    Console.WriteLine("The provided format of the private key is not supported (must be PEM or PFX) or the provided cert password is wrong");
                    return false;
                }
            }
            else
            {
                if (string.IsNullOrEmpty(newCertFormat))
                {
                    Console.WriteLine("There is no application certificate we can update and for the new application certificate there was not usable private key (must be PEM or PFX format) provided or the provided cert password is wrong");
                    return false;
                }
            }
            ...
            {
                Console.WriteLine("Remove the existing application certificate");
                try
                {
                    if (hasApplicationCertificate && !await appStore.Delete(currentApplicationCertificate.Thumbprint).ConfigureAwait(false))
                    {
                        Console.WriteLine($"Removing the existing application certificate with thumbprint '{currentApplicationCertificate.Thumbprint}' failed");
                    }
                }
                catch
                {
                    Console.WriteLine("Failed to remove the existing application certificate from the ApplicationCertificate store");
                }
                try
                {
                    await appStore.Add(newCertificateWithPrivateKey).ConfigureAwait(false);
                    Console.WriteLine($"The new application certificate '{newCertificateWithPrivateKey.SubjectName.Name}' and thumbprint '{newCertificateWithPrivateKey.Thumbprint}' was added to the application certificate store");
                }
                catch (Exception e)
                {
                    Console.WriteLine(e.Message, "Failed to add the new application certificate to the application certificate store");
                    return false;
                }
            }
            ...
            try
            {
                Console.WriteLine($"Activating the new application certificate with thumbprint '{newCertificateWithPrivateKey.Thumbprint}'");
            ...
            {
                Console.WriteLine(e.Message, "Failed to activate the new application certificate");
                return false;
            }
