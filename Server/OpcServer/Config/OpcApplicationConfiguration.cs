...
namespace OpcPlc
{
    ...
    public partial class OpcApplicationConfiguration
    {
        ...
        public static string HostnameLabel => _hostname.Contains('.')
                                                ...
                                                : _hostname;
        public static string ProgramName = "opc server";
        ...
        public ApplicationConfiguration Configure()
        {
            ...
            ApplicationConfiguration = InitApplicationSecurityAsync(securityBuilder);
            ...
            if (certificate == null)
            {
                Console.WriteLine($"No existing application certificate found. Creating a self-signed application certificate valid since yesterday for {CertificateFactory.DefaultLifeTime} months, " +
                    $"with a {CertificateFactory.DefaultKeySize} bit key and {CertificateFactory.DefaultHashSize} bit hash");
            }
            else
            {
                Console.WriteLine($"Application certificate with thumbprint {certificate.Thumbprint} found in the application certificate store");
            }

            // check the certificate, creates new self signed certificate if required
            bool isCertValid = application.CheckApplicationInstanceCertificate(silent: true, CertificateFactory.DefaultKeySize, CertificateFactory.DefaultLifeTime).Result;
            isCertValid = application.CheckApplicationInstanceCertificate(false, 0).Result;
